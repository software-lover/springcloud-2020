...
11、Sentinel
    Hystrix 需要我们程序员自己手工搭建监控平台；没有一套 web 界面可以给我们进行更加细粒度化的配置流控、速率控制，服务熔断、服务降级
    Sentinel 单独的一个组件，可以独立出来；直接界面化的细粒度统一配置

java -jar sentinel-dashboard-1.7.2.jar  即可启动   http://localhost:8080  即可访问  用户名密码都是 sentinel

sentinel 采用的是懒加载的方式，并不是服务启动后，就会在 sentinel 监控平台看到服务信息，而是需要访问一下服务接口才会在监控平台看到
